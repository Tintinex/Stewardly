...
import { handleCreateThread } from './handlers/create-thread'
...
export async function route(event: LambdaEvent): Promise<r.ApiResponse> {
  ...
  if (method === 'POST' && boardId && !threadId)    return handleCreateThread(event.body ?? null, hoaId, boardId, userId, role)
